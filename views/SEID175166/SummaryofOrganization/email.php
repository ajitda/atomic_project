...
<div class="container">
    ...
    <form  role="form" method="post" action="email.php<?php if(isset($_REQUEST['id'])) echo "?id=".$_REQUEST['id']; else echo "?list=1";?>">
        <div class="form-group">
            ...
            <input type="text"  name="subject"  class="form-control" id="subject" value="<?php if($list){echo "Summary of Organization recommendation";} else {echo "A single Hobby recommendation";} ?>">
...
if($list){
    ...
    $trs="";
    $sl=0;
    printf("<table><tr> <td width='50'><strong>Serial</strong></td><td width='50'><strong>ID</strong></td><td width='250'><strong>Company Name</strong></td><td width='550'><strong>Summary</strong></td></tr>");
    foreach($recordSet as $row) {

        $id = $row->id;
        $companName = $row->company_name;
        $summary = $row->summary;

        $sl++;
        printf("<tr><td width='50'>%d</td><td width='50'>%d</td><td width='250'>%s</td><td width='550'>%s</td></tr>",$sl,$id,$companName,$summary);
...
{
    printf("I'm recommending You: [<strong>Company ID: </strong>%d, <strong>COmpany Name: </strong>%s, <strong>Summary: </strong>%s]",$singleItem->id,$singleItem->company_name,$singleItem->summary);
