...
if(!isset($_REQUEST['id'])) {
    $list = 1;
    $recordSet = $Hobbies->index();

}
else {
    $list= 0;
    $Hobbies->setData($_REQUEST);
    $singleItem = $Hobbies->view();
}
...
<div class="container">
    ...
    <form  role="form" method="post" action="email.php<?php if(isset($_REQUEST['id'])) echo "?id=".$_REQUEST['id']; else echo "?list=1";?>">
        <div class="form-group">
            ...
            <input type="text"  name="subject"  class="form-control" id="subject" value="<?php if($list){echo "List of Hobbies recommendation";} else {echo "A single Hobby recommendation";} ?>">
...
$sl=0;
    ...
    printf("<table><tr> <td width='50'><strong>Serial</strong></td><td width='50'><strong>ID</strong></td><td width='250'><strong>Name/strong></td><td width='250'><strong>Hobbies</strong></td></tr>");
    ...
    $sl++;
    printf("<tr><td width='50'>%d</td><td width='50'>%d</td><td width='250'>%s</td><td width='250'>%s</td></tr>",$sl,$id,$Name,$Hobbies);
...
{
    printf("I'm recommending You: [<strong>Hobbies ID: </strong>%s, <strong>Name: </strong>%s, <strong>Hobbies: </strong>%s]",$singleItem->id,$singleItem->name,$singleItem->hobbies);
